  ...
  (selectizeConfig, $timeout) ->
    {
      ...
      link: (scope, element, attrs, modelCtrl) ->
        ...
        setSelectizeOptgroups = (curr, prev) ->
          angular.forEach prev, (opt) ->
            if curr.indexOf(opt) == -1
              value = opt[settings.optgroupValueField]
              selectize.removeOptionGroup value
            return
          angular.forEach curr, (opt) ->
            value = opt[settings.optgroupValueField]
            # selectize.addOptionGroup value, opt
